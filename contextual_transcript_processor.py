...
if __name__ == "__main__":
    # Test de récupération de transcript d'abord
    video_id = "39nGlTNPmpE"
    
    # Debug: voir la structure du transcript
    try:
        api = YouTubeTranscriptApi()
        transcript = api.fetch(video_id)
        
        print("Type d'objet transcript:", type(transcript))
        print("Méthodes/attributs disponibles:")
        print([attr for attr in dir(transcript) if not attr.startswith('_')])
        
        # Test avec différentes structures possibles
        if hasattr(transcript, 'snippets'):
            print("\nPremiers segments (via snippets):")
            for i, snippet in enumerate(transcript.snippets[:3]):
                end_time = snippet.start + snippet.duration
                print(f"{snippet.start:.2f}s - {end_time:.2f}s: {snippet.text}")
        
    except Exception as e:
        print(f"Erreur debug: {e}")
    
    # Utilisation normale du processeur
    processor = ContextualTranscriptProcessor(api_key="your-openai-api-key")
    
    # Test de récupération
    segments = processor.get_transcript(video_id)
    if segments:
        print(f"\nTranscript récupéré avec succès: {len(segments)} segments")
        print("Premiers segments:")
        for segment in segments[:3]:
            print(f"{segment['start']:.2f}s: {segment['text']}")
    
    # Poser une question si le transcript fonctionne
    if segments:
        current_time = 145.5  # 2:25 dans la vidéo
        question = "Je ne comprends pas ce qu'il explique"
        
        response = processor.ask_question(video_id, current_time, question)
        print(f"\nRéponse IA:\n{response}")